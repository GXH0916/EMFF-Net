...
def structure_loss(pred, mask):
    weit = 1 + 5*torch.abs(F.avg_pool2d(mask, kernel_size=31, stride=1, padding=15) - mask)   # 1 + 5*torch.abs(F.avg_pool2d(mask, kernel_size=31, stride=1, padding=15) - mask)
...
def train(train_loader, model, optimizer, epoch):
    ...
    if (epoch+1) % 10 == 0:
        torch.save(model.state_dict(), save_path + 'EMFFNet-%d.pth' % epoch)
        print('[Saving Snapshot:]', save_path + 'EMFF-%d.pth'% epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', type=str,
                        default=r'C:\Users\inspur\Downloads\EMFFNet\data\TrainDataset', help='path to train dataset')
    parser.add_argument('--train_save', type=str,
                        default='EMFFNet-Res2Net')
